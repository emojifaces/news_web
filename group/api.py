        ...
        if request.user.is_authenticated:
            ...
            if iscollect == '1':
                ...
                if not collect_blog:
                    ...
                    return Response({'success': False, 'msg': '之前收藏过'})
            elif iscollect == '0':
                collect_blog = CollectBlogs.objects.filter(blog_id_id=blog_id, user_id_id=baseuser.id)
                if collect_blog:
                    CollectBlogs.objects.get(blog_id_id=blog_id, user_id_id=baseuser.id).delete()
                else:
                    return Response({'success': False, 'msg': '之前没收藏过 不能取消'})
            return Response({'success': True})
        else:
            return Response({'success': False, 'msg': '未登录'})
...
class AddFirstComment(APIView):
    ...
    def post(self, request):
        ...
        if not request.user.is_authenticated:
            ...
            baseuser = BaseUser.objects.get(auth_user=request.user)
            fbc = FirstBlogComment.objects.create(blog_id_id=blog_id, user_id_id=baseuser.id,
                                                  content=emoji.demojize(content))

            Remind.objects.create(user_id=baseuser, blog_id_id=blog_id, content=emoji.demojize(content),
                                  author=blog.user_id, type=0, first_comment=fbc)
...
class AddSecondComment(APIView):
    ...
    def post(self, request):
        ...
        if not request.user.is_authenticated:
            ...
            sbc = SecondBlogComment.objects.create(blog_id_id=blog_id, first_comment_id=first_id,
                                                   user_id_id=baseuser.id, content=content, reply_id_id=reply_id)
            data = {
                'success': True,
                'userId': baseuser.id,
                'comment': emoji.emojize(content),
            ...
            Remind.objects.create(user_id=baseuser, blog_id=blog, author_id=reply_id, type=1,
                                  content=emoji.demojize(content), )
            return Response(data)
...
class Publish(APIView):
    ...
    def post(self, request):
        ...
        if not request.user.is_authenticated:
            return Response({"success": False, "msg": "Not logged in"})
        else:
            if not content:
                return Response({"success": False, "msg": "Content cannot be empty"})
            else:
                content = emoji.demojize(content)
                baseuser = BaseUser.objects.get(auth_user=request.user)

                if vote_date:
                    blog = Blogs.objects.create(type=1, content=content, user_id_id=baseuser.id)
                    blog_id = blog.id
                    if len(vote_date) >= 2:
                        for vote_option in vote_date:
                            BlogChoices.objects.create(blog_id_id=blog_id, content=vote_option)
                    else:
                        return Response({"msg": '投票选项至少两个'})
                else:
