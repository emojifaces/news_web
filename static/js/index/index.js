    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            for (let data of result.data) {
                let div = $('<div class="main-data-div color-comment fastinfo"></div>')
                ...
                if (data.fast_type == 0) {
                    ...
                    for (let lightStar = 0; lightStar < data.star; lightStar++) {
                        let lightStarDiv = $('<svg t="1583754430858" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2559" width="16" height="16">\n' +
                            ...
                            ' p-id="2560" fill="' + star_color + '"></path>\n' +
                            '</svg>')
                    ...
                    let tag = $('<div class="cal-impact"></div>')
                    if (data.tag == 0) {
                        tag.addClass('tag-null')
                        tag.text('---')
                    } else if (data.tag == 1) {
                        tag.addClass('bear')
                        tag.text('BEAR')
        ...
        beforeSend: function () {
            $('.fastinfo-load-div').show()
        },
        complete: function () {
            $('.fastinfo-load-div').hide()
        }
    })
...
$(document).on('click', '#moreSummaryBtn', function () {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            for (let data of result.data) {
                let div = $('<div class="main-data-div color-comment fastinfo summary-info"></div>')
                let icon = $('<div class="main-data-icon">\n' +
                    '             <div class="icon-div summary-icon">\n' +
                    '             <img src="/static/images/7_24.png" alt="">' +
                    '             </div>\n' +
                    '         </div>')
                let time = $('<div class="main-data-time">' + removeMS(data.VN_pub_date) + '</div>')
        ...
        beforeSend: function () {
            $('.summary-load-div').show()
        },
        complete: function () {
            $('.summary-load-div').hide()
        }
    })
...
week_list = new Array("chủ nhật", "thứ hai", "thứ ba", "thứ tư", "thứ năm", "thứ sáu", "thứ bảy", "chủ nhật");
...
setInterval(function () {
    ...
    let localOffset = datetime.getTimezoneOffset() * 60000;   //getTimezoneOffset()返回是以分钟为单位，需要转化成ms
    let utc = localTime + localOffset;
    let offset = 7; //以越南时间为例，东7区
    let Vietnam = utc + (3600000 * offset);
    let Vietnam_time = new Date(Vietnam);


    // calendar模块中的倒计时功能
    let nexttime = $('#card-calendar-body').children('div:first-child').children('div:first-child').children('.calendar-time').text()
    if (!nexttime) {
        let remaining_time = 'Dữ liệu tiếp theo:  00:00:00'
        $('#remaining-time').text(remaining_time)
    } else {
        let nexttime_list = nexttime.split(':')
        let lefttime = new Date(Vietnam_time.getFullYear(), Vietnam_time.getMonth(), Vietnam_time.getDate(), parseInt(nexttime_list[0]), parseInt(nexttime_list[1]), parseInt(nexttime_list[2])) - Vietnam_time
        if (lefttime <= 0) {
            $('#card-calendar-body').children('div:first-child').remove();
            let remaining_time = 'Dữ liệu tiếp theo: 00 H 00 M 00 S'
            $('#remaining-time').text(remaining_time)
        } else {
            ...
            let remaining_time = 'Dữ liệu tiếp theo:  ' + lefthour + ':' + leftminute + ':' + leftsecond
            $('#remaining-time').text(remaining_time)
        }
            ...
            if (!result.success) {
                ...
                for (let data of result.data) {
                    ...
                    let country_title = $('<div>\n' +
                        ...
                        '      <div class="countryCode">' + data.tran_country + '</div>\n' +
...
function getBlogList() {
    $.ajax({
        ...
        success: function (result) {
            ...
            if (result.success != true) {
                ...
                for (let data of groupList) {
                    ...
                    let discuss = $('<div class="discuss expand-comment" id="all-comments">\n' +
                        '               <img src="/static/images/comment1.png" alt="">\n' +
                        '               <span class="color-comment">' + data.commentnum + '</span>\n' +
                        '            </div>')
                    let like = $('<div class="like"></div>')
                ...
                let moreBlogBtn = $('<div class="main-data-div">\n' +
                    ...
                    '<div class="main-footer cursor-pointer"  id="indexMoreGroup" >Xem thêm</div>\n' +
                    '</div>\n' +
                    '</div>')
        ...
        beforeSend: function () {
            $('.index-group-load-div').show()
        },
        complete: function () {
            $('.index-group-load-div').hide()
            $('.right-user-dynamic-scroll-box').find('.user-dynamic-box').each(function () {
...
index_group_page = 1
index_group_limit = 30
// index 加载更多group
$(document).on('click', '#indexMoreGroup', function () {
    let moreBtn = $(this).parents('.main-data-div')
    let offset = moreBtn.parent('.right-user-dynamic-scroll-box').find('.user-dynamic-box').length
    moreBtn.remove()
    $.ajax({
        type: 'get',
...
$(document).on('click', '.expand-comment', function () {
    ...
    if (!_this.hasClass('expand')) {
        ...
        _this.parents('.dynamic-info').siblings('.dynamic-comment-box').remove()
                    ...
                    if (fbc.sbc) {
                        ...
                        if (fbc.sbc_num > 2) {
                            let moreSBCBtn = $('<div class="more-sbc">' +
                                '<img src="/static/images/morecomment2.png" alt="">&nbsp;&nbsp;<span>(' + (fbc.sbc_num - 2) + ')</span>' +
                                '</div>')
                            fbc_commentDate.append(moreSBCBtn)
                        }
                    }
